...
async def _afficher_ranking(session: SessionDiagnostic, agent: AgentDiagnostic):
    """Computes and displays the current CIF ranking after every agent message."""
    if not session.fiche_id:
        return
    ranking = agent.appel_ranking_cif_public(session)
    if not ranking:
        return
    lignes = ["**🔍 Current fault candidates:**"]
    for item in ranking:
        prob = item.get("probabilite", "?")
        perimeter = item.get("perimeter", "")
        titre = item.get("cif_title", "")
        lignes.append(f"- **{prob}%** — {perimeter} / {titre}")
    await cl.Message(content="\n".join(lignes), author="Diagnostic assistant").send()
...
async def recevoir_message(message: cl.Message):
    ...
    if session.etape == EtapeDiagnostic.TERMINE:
        await cl.Message(content="This session is already complete. Type **/new** to start a new fault report.").send()
        return
    ...
    await cl.Message(content=reponse).send()
    await _afficher_ranking(session, agent)
    ...
    if session.etape == EtapeDiagnostic.TERMINE:
        ...
        await cl.Message(
            ...
            actions=[cl.Action(name="new_report", label="🔄 Start new report", payload={"action": "new"})]
        ).send()
...
async def _afficher_bouton_fin():
    await cl.Message(
        content=" ",
        actions=[cl.Action(name="finish_complements", label="✅ I have nothing more to add", payload={"action": "done"})]
    ).send()
...
async def _afficher_recap(session: SessionDiagnostic):
    ...
    if data.get("cif_ranking"):
        ...
        for item in data["cif_ranking"]:
            lignes.append(f"- **{item.get('probabilite', '?')}%** — {item.get('perimeter', '')} / {item.get('cif_title', '')}")
        ranking_texte = "\n".join(lignes)
    recap = f"""
